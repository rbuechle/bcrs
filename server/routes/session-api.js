...
router.post('/signin', function (req, res, next) {
  User.findOne({ username: req.body.username }, function (err, user) {
    if (err) {
       ...
       */
      if (user) {
        const passwordIsValid = bcrypt.compareSync(req.body.password, req.user.password) // compare the saved hashed password against the signin password
...
router.post('/verify/users/:username/security-questions', function (req, res, next) {
  ...
  User.findOne({ username: req.params.username }, function (err, user) {
    if (err) {
      ...
      const answer1IsValid = answerToSecurityQuestion1 === user.securityQuestions[0].answer
      console.log(answer1IsValid)

      const answer2IsValid = answerToSecurityQuestion2 === user.securityQuestions[1].answer
      console.log(answer2IsValid)

      const answer3IsValid = answerToSecurityQuestion3 === user.securityQuestions[2].answer
...
router.post('/users/:username/reset-password', function (req, res, next) {
  ...
  User.findOne({ username: req.params.username }, function (err, user) {
    if (err) {
      ...
      const hashedPassword = bcrypt.hashSync(password, saltRounds) // salt/hash the password
